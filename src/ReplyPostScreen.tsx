...
const ReplyPostScreen = () => {
  ...
  useEffect(()=> {
    ...
    let unique: any = [];
    duplicate = arr.filter((item, index)=> arr.indexOf(item) != index)
    arr.filter((item, index)=> arr.indexOf(item) == index ? unique.push(item) : null)

    let result: any = [];
    unique.map((item: any)=> !duplicate.includes(item) ?  result.push(item) : null)
  ...
  useEffect(()=> {
    ...
    let result: any = [];
    arr.map((input, index)=> {
      let existing = arr.find(()=> index == arr.findIndex((item) => item.id == input.id))
      if(existing) {
        result.push(input)
      }
    })
  ...
  useEffect(()=> {
    ...
    new Array(100).fill(0).map((_, index)=> {
      let fizz = index % 3 == 0;
      let buzz = index % 5 == 0;
      fizz ? ( buzz ? result.push('FizzBuzz') : result.push('Fizz') ) : buzz ? result.push('Buzz') : result.push(index);
    });
      ...
      <View style={styles.bottomBox}>
        <View style={styles.postBody}>
          ...
          <View style={[styles.bodyBox, styles.rowSection]}>
            <Text style={[styles.subHeading, styles.titleColor]}>Reply to an online post</Text>
            <AntDesignIcons name="sound" size={20} color="#900" style={[styles.subHeading]} />
          </View>
          <View style={[styles.rowSection,  {justifyContent: 'flex-start'}]}>
            <MaterialIcons name="watch-later" size={14} color="#900" />
            <Text style={[styles.heading, styles.titleColor]}>3 minutes</Text>
          </View>
          <View style={[styles.rowSection,  {justifyContent: 'flex-start'}]}>
            <MaterialIcons name="keyboard" size={14} color="#900" />
            <Text style={[styles.heading, styles.titleColor]}>Write at atleast 40 words</Text>
          </View>
          <View style={styles.bodyBox}>
            <Text style={styles.body}>Read a question posted on a online discussion website. Write a response with advice.</Text>
            <Text style={[styles.body, styles.bodyBox]}>You will have 3 minutes to read and respond.</Text>
